...
import 'package:yaantrac_app/TMS/presentation/screen/tire_graph.dart';
...
class TirePerformanceScreen extends StatefulWidget {
  final TireModel tire;
  const TirePerformanceScreen({super.key, required this.tire});

  @override
  State<TirePerformanceScreen> createState() => _TirePerformanceState();
}

class _TirePerformanceState extends State<TirePerformanceScreen> {
                                                ...
                                                if (_formKey.currentState!
                                                  ...
                                                  ctx
                                                      .read<
                                                          BaseCubit<
                                                              TirePerformanceModel>>()
                                                      .addPerformance(tirep,
                                                          endpoint:
                                                              '/tires/${widget.tire.id}/add-performance',
                                                          fetchApi:
                                                              '/tires/${widget.tire.id}/performances');
                                                  Navigator.pop(context);
                                                }
                                              },
                                              title: "Add"))
  ...
  String _calculateAverage(List<TirePerformanceModel> tirePerformances,
      double Function(TirePerformanceModel) selector) {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        body: BlocConsumer<BaseCubit<TirePerformanceModel>,
            BaseState<TirePerformanceModel>>(
          ...
          builder: (context, state) {
            ...
            } else if (state is ErrorState<TirePerformanceModel>) {
              return Center(
                child: Text(state.message.toString()),
              );
            } else if (state is LoadedState<TirePerformanceModel>) {
              final tirePerformances = state.items;
  ...
  Widget _buildStatCard(
      List<TirePerformanceModel> tire, String title, ThemeData theme) {
    return Card(
      elevation: 2.h,
